...
public sealed class NpgSqlSession(NpgSqlDataSourceFactory factory) : IDisposable, IAsyncDisposable
{
    ...
    public void ApplyTransaction<T>(Action<T, IDbTransaction> application, T source) where T : notnull
    {
        if (_transaction == null) return;
        application(source, _transaction);
    }
    ...
    {
        return _connection ??= await factory.CreateConnection(ct);
    }
}