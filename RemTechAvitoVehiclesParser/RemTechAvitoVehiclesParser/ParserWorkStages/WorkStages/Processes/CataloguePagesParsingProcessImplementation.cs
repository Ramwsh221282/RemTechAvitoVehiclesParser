...
public static class CataloguePagesParsingProcessImplementation
{
    ...
    {
        public static WorkStageProcess CatalogueProcess => async (deps, ct) =>
        {
            ...
            PaginationParsingParserLink link = parser.Value.Links[0];
        ...
        )
    {
        ...
        try
        {
            ...
            if (!await deps.Bypasses.Create(await browser.GetPage()).Bypass()) return url;
            ...
            await foreach ((string Id, string Url, IReadOnlyList<string> Photos) in GetCatalogueItemsMetadata(catalogueElements, await browser.GetPage()))
